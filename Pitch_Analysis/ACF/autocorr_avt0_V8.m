...
function [f0, xaxis, t0, bsig] = autocorr_avt0_V8(x,fs,fsize,fshift,plotFlag, dflag)
...
for i=1:c
    ...
    %   neibouring frame rule...
    if ((~(Pavt0-10<MaxPS && MaxPS<Pavt0+10)) && i>1)
    ...
    while(((bval<endPeakP) || (nextpos<=NPeaks)) && k<=3)
        ...
        if (k==3 && i>=2)
            ...
            if (k<3)
                ...
                if ((locPeaks(remPer(maxpos))-maxPeakP(2)>=(maxPeakP(2)-maxPeakP(1)-5)) && (locPeaks(remPer(maxpos))-maxPeakP(2)<=(maxPeakP(2)-maxPeakP(1)+5)))
                    maxPeakP(k) = locPeaks(remPer(maxpos));
    ...
    avt0(i) = mean(diff([0, maxPeakP]));                                    %In Samples
