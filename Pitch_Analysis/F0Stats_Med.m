...
% --------------------------------------------------------------------------
function [cryMeanf0, cryDevf0, caseMedf0, caseMedDevf0, NDevf0, f01v, xaxis1, begP, endP] = F0Stats_Med(cryFile)
...
vind = find(vas);
if (vind(end)>size(f01v, 2))
    vind(end) = size(f01v, 2);
end    
f01v(vind) = f01(vind);
...
for i = 1:cLen
    curCry = f01v(1, begP(i):endP(i));    
    valNum = curCry(find(~isnan(curCry)));
    if (isempty(valNum))
        cryMeanf0(1, i) = NaN;
        cryDevf0(1, i) = NaN;
        continue;
    end
    cryMeanf0(1, i) = mean(valNum);
    cryDevf0(1, i) = std(valNum);
end

% Calculate the sample Mean

caseMedf0 = median(cryMeanf0(~isnan(cryMeanf0)));
caseMedDevf0 =  median(cryDevf0(~isnan(cryDevf0)));
NDevf0 = caseMedDevf0/caseMedf0;
end
